...
def sync_busy_data(request):
    ...
    try:
        ...
        for p in parties:
            ...
            name = p.get('name')
            alias = p.get('alias')
            BusyParty.objects.update_or_create(
                code=code,
        ...
        if party_codes:
            ...
            BusyLedgerEntry.objects.bulk_create(bulk_entries)
        ...
        for p in all_parties:
            ...
            if "," in alias:
                ...
                if code.startswith("DLR-"):
                    # Update Dealer
                    dealer = Dealer.objects.filter(dealercode__iexact=code).first()
                    if dealer:
                        bal = BusyLedgerEntry.objects.filter(party=p).aggregate(total=Sum('amount'))['total'] or 0
                        dealer.outstanding = bal
                        dealer.save()
                elif code.startswith("DST-"):
                    # Update Distributor
                    dist = Distributor.objects.filter(distributorcode__iexact=code).first()
                    if dist:
                        bal = BusyLedgerEntry.objects.filter(party=p).aggregate(total=Sum('amount'))['total'] or 0
...
def search_busy_parties(request):
    try:
        ...
        parties = BusyParty.objects.filter(name__icontains=query)[:20]
