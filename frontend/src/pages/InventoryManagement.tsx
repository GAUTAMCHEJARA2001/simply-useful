...
// ─── INV API Helper (Real implementation using apiClient) ──────────────
const inv = {
  get: <T,>(path: string): Promise<T> => apiClient<T>(`/${path}`),
  post: <T,>(path: string, data: any): Promise<T> => apiClient<T>(`/${path}`, { data }),
  del: (path: string): Promise<any> => apiClient<any>(`/${path}`, { method: 'DELETE' }),
  put: <T,>(path: string, data: any): Promise<T> => apiClient<T>(`/${path}`, { method: 'PUT', data }),
};
  ...
  return (
    <div className="flex flex-col md:flex-row gap-6 relative">
      ...
      <div className="flex-1 min-w-0">
        ...
        {tab === 'total_stock' && (
          <SafeDataView data={aggregateStock} isLoading={loading} error={error} onRetry={() => loadTabData('total_stock')} emptyMessage="No stock data found" renderItem={() => (
            <TotalStockTab aggregateStock={aggregateStock} stock={stock} />
          )} />
        )}
        {tab === 'units' && (
          <SafeDataView data={units} isLoading={loading} error={error} onRetry={() => loadTabData('units')} emptyMessage="No units found" renderItem={() => (
        ...
        {tab === 'suppliers' && (
          <SafeDataView data={suppliers} isLoading={loading} error={error} onRetry={() => loadTabData('suppliers')} emptyMessage="No suppliers found" renderItem={() => (
        ...
        {tab === 'labour' && (
          <SafeDataView data={labours} isLoading={loading} error={error} onRetry={() => loadTabData('labour')} emptyMessage="No labour found" renderItem={() => (
        ...
        {tab === 'purchase_orders' && (
          <SafeDataView data={purchaseOrders} isLoading={loading} error={error} onRetry={() => loadTabData('purchase_orders')} emptyMessage="No purchase orders found" renderItem={() => (
        ...
        {tab === 'purchases' && (
          <SafeDataView data={purchases} isLoading={loading} error={error} onRetry={() => loadTabData('purchases')} emptyMessage="No purchases found" renderItem={() => (
        ...
        {tab === 'sales' && (
          <SafeDataView data={sales} isLoading={loading} error={error} onRetry={() => loadTabData('sales')} emptyMessage="No sales found" renderItem={() => (
        ...
        {tab === 'productions' && (
          <SafeDataView data={productions} isLoading={loading} error={error} onRetry={() => loadTabData('productions')} emptyMessage="No production records found" renderItem={() => (
        ...
        {tab === 'adjustments' && (
          <SafeDataView data={adjustments} isLoading={loading} error={error} onRetry={() => loadTabData('adjustments')} emptyMessage="No adjustments found" renderItem={() => (
        ...
        {tab === 'attendance' && (
          <SafeDataView data={attendance} isLoading={loading} error={error} onRetry={() => loadTabData('attendance')} emptyMessage="No attendance records found" renderItem={() => (
        ...
        {tab === 'approvals' && (
          <SafeDataView data={approvals} isLoading={loading} error={error} onRetry={() => loadTabData('approvals')} emptyMessage="No pending approvals found" renderItem={() => (
        ...
        {tab === 'returns' && (
          <SafeDataView data={returns} isLoading={loading} error={error} onRetry={() => loadTabData('returns')} emptyMessage="No returns found" renderItem={() => (
            <ReturnsTab returns={returns} onRowClick={r => handleRowClick('return', r)} Currency={Currency} />
          )} />
        )}
        {tab === 'reports' && (
          <SafeDataView data={stock} isLoading={loading} error={error} onRetry={() => loadTabData('reports')} renderItem={() => (
